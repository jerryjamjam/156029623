...
// i.e NQXPOMAFTRHLZGECYJIUWSKDVB = the alphabet
...
int main(int argc, string argv[])
{
    ...
    //creates array "key" then checks and stores each char in array key, then checks for dupes. if key[i] is greater than 1, that means theres a dupe
    int dupecheck[52] = {0};
    for(int i = 0, j = strlen(argv[1]); i < j; i++)
    if(islower(argv[1][i]))
    {
